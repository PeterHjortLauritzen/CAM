...
subroutine print_budget(hstwr)
  ...
  if (masterproc .and. thermo_budget_history .and. hstwr(thermo_budget_histfile_num)) then
    ...
    do i=1,4
      ...
      call cam_budget_get_global('phAF-phAC',idx(i),dEdt_dme_adjust_physE(i))
      ...
      call cam_budget_get_global('dyAF-dyAC',idx(i),dEdt_dme_adjust_dynE(i))
...
!        end if
    ...
    do m_cnst=1,thermo_budget_num_vars
      if (thermo_budget_vars_massv(m_cnst)) then
        ...
        call cam_budget_get_global('phAF-phAC',m_cnst,dMdt_dme_adjust)
