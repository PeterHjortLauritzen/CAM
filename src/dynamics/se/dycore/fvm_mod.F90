...
!  type (EdgeBuffer_t), private  :: ghostBufFlux_h
  ...
  type (EdgeBuffer_t), public  :: edgeBuf_cslam2gll_dss !DSS of Qdp on GLL after cslam2gll
...
!    real (kind=r8)               :: ftmp(1-nhcc:nphys+nhcc,1-nhcc:nphys+nhcc,numlev,num_flds,nets:nete)
  ...
  subroutine fvm_init2(elem,fvm,hybrid,nets,nete)
    ...
    !
    ! 1-deep halo buffer for the CSLAM Q filter (apply_cslam_q_filter_del2/del4):
    ! the filter's plus-stencil only reads halo ring 1, so exchanging the full
    ! nhc-deep halo (ghostBufQnhc) is wasted bandwidth.  2*nlev layers hold
    ! dp_fvm and Q in a single exchange; the L exchange (nlev layers at
    ! kptr=kmin-1) and both call sites (full 1:nlev and jet kmin_jet:kmax_jet)
    ! fit within 2*nlev since kmax <= nlev.
    !
...
!    call initghostbuffer(hybrid%par,ghostBufFlux_h,elem,4*nlev,nhe,nc,nthreads=horz_num_threads)
    ...
    call initEdgeBuffer(hybrid%par,edgeBuf_cslam2gll_dss,elem,nlev*thermodynamic_active_species_num,nthreads=1)
    !
