...
! Private module data
   ...
   real(r8) :: n2vmr  = -1.0_r8                ! n2    volume mixing ratio
...
subroutine chem_surfvals_readnl(nlfile)
   ...
   namelist /chem_surfvals_nl/ co2vmr, n2ovmr, n2vmr, ch4vmr, f11vmr, f12vmr, &
   ...
   call mpi_bcast(n2vmr, 1, mpi_real8, mstrid, mpicom, ierr)
   if (ierr /= 0) call endrun(sub//" mpi_bcast: n2vmr")
...
!-----------------------------------------------------------------------
   ...
   if (masterproc) then
      ...
      write(iulog,*) '  n2o volume mixing ratio = ', n2vmr
...
!---------------------------Local variables-----------------------------
   ...
   if (masterproc .and. is_end_curr_day()) then
      ...
      if (.not. ramp_just_co2 .and. mon==1 .and. day==1) then
         write(iulog,*) 'chem_surfvals_set: ch4vmr=', ch4vmr, ' n2ovmr=', n2ovmr, ' n2vmr=', n2vmr, &
