...
echo $PBS_ACCOUNT
unset caze
set caze=${cset}_${src}_${res}
/glade/scratch/$USER/$src/cime/scripts/create_newcase --case $scratch/$USER/$caze --compset $cset --res $res  --q $queue --walltime 01:30:00 --pecount $pecount  --project $PBS_ACCOUNT --compiler $compiler --run-unsupported
#$homedir/$USER/src/$src/cime/scripts/create_newcase --case $scratch/$USER/$caze --compset $cset --res $res  --q $queue --walltime 00:30:00 --pecount $pecount  --project $PBS_ACCOUNT --compiler $compiler --run-unsupported
...
if ( $res == "ne30pg3_ne30pg3_mg17" || $res == "ne30_ne30_mg17" ) then
  ...
  echo "fincl1='OMEGA500','OMEGA850','Z500','Z300','Z200','Z100','Z3' "    >> user_nl_cam
endif
