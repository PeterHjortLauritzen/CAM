...
!+tht
   ...
   !+tht
       call set_dry_to_wet(state)    ! Physics had dry, dme_adj wants moist
       call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt &
                              ...
                              , ocnfrac=cam_in%ocnfrac, sst=cam_in%sst, ts=cam_in%ts) !+tht
       call set_wet_to_dry(state)    ! back to dry
       call check_energy_chng(state, tend, "enthalpy_evap_tend", nstep, ztodt, zero, zero, zero, eflx)
      !tht: call to thermo_water may be limited to end of physics cycle
   ...
   !tht: after dme_adjust, must reset the qini's for the next call
...
#define l_tht
#define new !tht: c out to use l_tht option below
#ifdef new
...
! beg not new
    ...
    moist_mixing_ratio_dycore = dycore_is('LR').or. dycore_is('FV3')
      ...
      call check_energy_chng(state, tend, "dme_adjust", nstep, ztodt, zero, zero, zero, eflx)
      !tht: thermo_water should be the last step of physics state update
      call cam_thermo_water_update(state%q(:ncol,:,:), lchnk, ncol, vc_dycore, &
           to_dry_factor=state%pdel(:ncol,:)/state%pdeldry(:ncol,:))
...
!+tht additional output from zm to pass to dme_bflx (dme_energy_adjust) 18/6/24
    ...
    real(r8) :: prec(pcols),snow(pcols)! prec and snow
!-tht
!+tht variables for dme_energy_adjust 
    real(r8) :: eflx(pcols), eflx_ocn(pcols), dsema(pcols), mxflx(pcols), myflx(pcols)
!-tht
...
!+tht
      !call enthalpy_zm_tend(pbuf,state,ptend,precl_enthalpy_flux) !tht c'd out
     !+tht:added 18/6/24
       if (difzm_idx > 0) then
     ...
     !-tht:added 18/6/24
     ! call set_dry_to_wet(state)    ! Physics had dry, dme_adj wants moist !! unnecessary?
       call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt &
     !+tht:added 18/6/24
                              , prec=prec, snow=snow &
     ...
     !-tht:added 18/6/24
                              , eflx=eflx, eflx_ocn=eflx_ocn, mxflx=mxflx, myflx=myflx, ent_tnd=dsema &
                              , ocnfrac=cam_in%ocnfrac, sst=cam_in%sst, ts=cam_in%ts) !+tht
     ! call set_wet_to_dry(state)    ! back to dry !! unnecessary?
       call check_energy_chng(state, tend, "enthalpy_zm_tend", nstep, ztodt, zero, zero, zero, eflx)
      !tht: call to thermo_water may be limited to end of physics cycle
      !call cam_thermo_water_update(state%q(:ncol,:,:), lchnk, ncol, vc_dycore, &
      !     to_dry_factor=state%pdel(:ncol,:)/state%pdeldry(:ncol,:))
       call outfld('EFLX'      ,  eflx, pcols, lchnk)
...
!! tht: qini's now changed here -- I don't quite understand why they're passed here as values before all phys_bc  
