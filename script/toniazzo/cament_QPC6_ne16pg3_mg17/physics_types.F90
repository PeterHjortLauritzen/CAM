...
!------------------------------Arguments--------------------------------
    ...
    logical, intent(in), optional :: really !+tht regtest: else do nothing
!
!---------------------------Local storage-------------------------------
   ...
   !if(.not.present(really)) return !+tht regtest
...
!+tht
    !use conserved energy
      call get_conserved_energy(.true., ptend%top_level, ptend%bot_level  &
      ...
      call inv_conserved_energy(.true., ptend%top_level, ptend%bot_level  &
                               , te(:ncol,:), cpairv_loc(:ncol,:), state%q(:ncol,:,:), state%pdel(:ncol,:) &
                               , pdel(:ncol,:), t_tmp(:ncol,:))
          if (present(tend)) &
      tend%dtdt(:ncol,ptend%top_level:ptend%bot_level)=(T_tmp(:ncol,ptend%top_level:ptend%bot_level) &
...
!+tht: gatekeeper module to control options for dme adjustment 
  subroutine physics_dme_adjust(state, tend, qini, liqini, iceini, dt &
     !+tht 18/6/24
                              ...
                              , dlf, dif &
     !-tht 18/6/24
                               , eflx, eflx_ocn, mxflx, myflx, ent_tnd &
                               , ocnfrac, sst, ts)
...
!+tht 18/6/24
       real(r8), intent(out), optional ::    prec(pcols     )   ! precipitation (liq+ice)
       real(r8), intent(out), optional ::    snow(pcols     )   ! snowfall
       real(r8), intent(out), optional :: ntprprd(pcols,pver)   ! net precip (liq+ice) production in layer
       real(r8), intent(out), optional :: ntsnprd(pcols,pver)   ! net snow production in layer
       real(r8), intent(out), optional ::     dlf(pcols,pver)   ! reserved water (liq+ice) prod in layer
       real(r8), intent(out), optional ::     dif(pcols,pver)   ! reserved ice prod in layer
!-tht
       real(r8), intent(out),  optional   :: eflx    (pcols)    ! energy flux for use in check_energy
       real(r8), intent(out),  optional   :: eflx_ocn(pcols)    ! energy flux for use in check_energy
       real(r8), intent(out),  optional   :: mxflx   (pcols)    ! boundary x-momentum flux 
       real(r8), intent(out),  optional   :: myflx   (pcols)    ! boundary y-momentum flux 
      ...
      ! just in local work space for now
       real(r8)           :: htx_cond(pcols,pver)  ! enthalpy tendency due to heat exchange with "condensates"
       real(r8)           :: mxx_cond(pcols,pver)  ! exchange x-momentum tendency
       real(r8)           :: myx_cond(pcols,pver)  ! exchange y-momentum tendency
       logical, parameter :: hydrostatic      =.true. ! condensates have surface specific enthalpy
       logical, parameter :: dme_ohf_adjust   =.true. ! condensates have surface specific enthalpy
       logical, parameter :: dme_adjust_to_mom=.true. ! condensates have surface specific enthalpy
       logical, parameter :: dme_omf_adjust   =.false.! condensates have surface specific enthalpy
       logical, parameter :: dme_hybrid_coord =.true. ! this routine returns new state on hybrid coords
       logical :: dme_energy_adjust

      call phys_getopts(dme_energy_adjust_out=dme_energy_adjust)
     ! eventually these also should be implemented instead of passing as
     ! optional args:
     !call phys_getopts(dme_hybrid_coord_out =dme_hybrid_coord )
     !call phys_getopts(dme_ohf_adjust_out   =dme_ohf_adjust   )
     !call phys_getopts(dme_adjust_to_mom    =adjust_to_mom    )
     !call phys_getopts(dme_omf_adjust_out   =dme_omf_adjust   )

      if (dme_energy_adjust) then
         call physics_dme_bflx(state, tend, qini, liqini, iceini, dt &
                              , eflx, mxflx, myflx, htx_cond, mxx_cond, myx_cond &
     !+tht: 18/6/24
                              , prec=prec, snow=snow &
                              , ntprprd=ntprprd, ntsnprd=ntsnprd &
                              , dlf=dlf, dif=dif &
     !-tht: 18/6/24
                              , hydrostatic=hydrostatic, dme_hybrid_coord=dme_hybrid_coord &
                              , ohf_adjust=dme_ohf_adjust, adjust_to_mom=dme_adjust_to_mom &
                              , omf_adjust=dme_omf_adjust &
                              , ocnfrac=ocnfrac, sst=sst, ts=ts, eflx_ocn=eflx_ocn)
         call physics_dme_adjust_THT(state, tend, qini, liqini, iceini, dt &
                              , htx_cond, mxx_cond, myx_cond &
                              , dlf=dlf, dif=dif & !+18/6/24
                              , ent_tnd=ent_tnd &
                              , dme_hybrid_coord=dme_hybrid_coord, hydrostatic=hydrostatic)
      else
           if (present(eflx))     eflx    (:)=0._r8
           if (present(eflx_ocn)) eflx_ocn(:)=0._r8
           if (present(ent_tnd))  ent_tnd (:)=0._r8
           if (present(mxflx))    mxflx   (:)=0._r8
           if (present(myflx))    myflx   (:)=0._r8
...
!-tht
!===============================================================================
  subroutine physics_dme_adjust_BAB(state, qini, liqini, iceini, dt) !tht: _BAB
...
!-----------------------------------------------------------------------
  subroutine physics_dme_bflx(state, tend, qini, liqini, iceini, dt &
                             , eflx, mxflx, myflx, htx_cond, mxx_cond, myx_cond &
     !+tht 18/6/24
                              , evap &
                              , prec, snow &
                              ...
                              , dlf, dif &
     !-tht 18/6/24
                             , hydrostatic , dme_hybrid_coord &
                             , ohf_adjust, adjust_to_mom &
                             , omf_adjust, ocnfrac, sst, ts, eflx_ocn)
    ...
    use dycore,          only: dycore_is ! might be rm'd when code is checked and cleaned up
    ...
    !     The option to return values on fixed-eta hybrid levels need to be
    !     consistent with dme_adjust.
    !
    ...
    real(r8),            intent(in   ) :: dt                 ! model physics timestep
!+18/6/24
       real(r8), intent(out), optional ::    evap(pcols     )! surf. evaporation/deposition (vapour)
       real(r8), intent(out), optional ::    prec(pcols     )! precipitation (liq+ice)
       real(r8), intent(out), optional ::    snow(pcols     )! snowfall
       real(r8), intent(out), optional :: ntprprd(pcols,pver)! net precip (liq+ice) production in layer
       real(r8), intent(out), optional :: ntsnprd(pcols,pver)! net snow production in layer
       ...
       real(r8), intent(out), optional ::     dif(pcols,pver)! reserved ice prod in layer
!-18/6/24
    real(r8), intent(out)           :: eflx    (pcols)       ! diagnostic: boundary enthalpy flux 
    real(r8), intent(out)           :: mxflx   (pcols)       ! diagnostic: boundary x-momentum flux 
    real(r8), intent(out)           :: myflx   (pcols)       ! diagnostic: boundary y-momentum flux 
    real(r8), intent(out)           :: htx_cond(pcols,pver)  ! diagnostic: exchange enthalpy tendency
    real(r8), intent(out)           :: mxx_cond(pcols,pver)  ! diagnostic: exchange x-momentum tendency
    real(r8), intent(out)           :: myx_cond(pcols,pver)  ! diagnostic: exchange y-momentum tendency
    logical , intent(in) , optional :: hydrostatic           ! flag to set energy function to hydrostatic  
    logical , intent(in) , optional :: ohf_adjust            ! flag to set enthalpy of water condensates 
    logical , intent(in) , optional :: adjust_to_mom         ! flag to set enthalpy of water condensates 
    logical , intent(in) , optional :: omf_adjust            ! flag to set momentum of water condensates 
    real(r8), intent(in) , optional :: ocnfrac(pcols)        ! Ocean fraction (fraction)
    real(r8), intent(in) , optional :: sst    (pcols)        ! Sea surface temperature
    real(r8), intent(in) , optional :: ts     (pcols)        ! Surface temperature
    logical , intent(in) , optional :: dme_hybrid_coord      ! Flag for hybrid (=T) or lagrangian (=F) coord
    real(r8), intent(out), optional :: eflx_ocn(pcols)       ! diagnostic: bndry enth. flux adj. to oc. ref. T
    ...
    real(r8) :: mdr     (pcols,pver) ! mass adjustment, regularized 
    ...
    real(r8) :: condeps    (pcols,pver)  ! specific enthalpy of moist reservoir with which q is exchanged
    ...
    real(r8) :: condeps_ocn(pcols,pver)  ! condeps, referenced to ocean T_0
...
!print*,'running dme_bflx'
    if (state%psetcols .ne. pcols) then
       call endrun('physics_dme_bflx: cannot pass in a state which has sub-columns')
    end if

    if(present(dme_hybrid_coord)) then 
     hybrid_coord=dme_hybrid_coord
    else
     hybrid_coord=.false.
    endif
   ...
   ! store old pressure
    ...
    state%ps(:ncol) = state%pint(:ncol,1) 
      ...
      call get_conserved_energy(.false. &               ! false for now, to be consistent with above
                               ,1 ,pver &
                               ,cpairv(:ncol,:,lchnk) & 
                               ,state%t(:ncol,:) ,state%q(:ncol,:,:) ,state%pdel(:ncol,:) &
                               ,pdel_new(:ncol,:) ,te(:ncol,:) &
                               ,phis=state%phis(:ncol) ,gph=state%zm(:ncol,:) &
                               ,U=state%u(:ncol,:) ,V=state%v(:ncol,:) &
                               ,latent=dummy,temce=emce,rairv=rairv(:ncol,:,lchnk))
    ...
    ! mass increments Dp'/Dp
     ...
     !+18/6/24: add back reserved liquid/ice to mdq?
   !   if (present(dlf)) &
   !    mdq(:ncol,k)=mdq(:ncol,k)+dlf(:ncol,k)
     !-18/6/24
       ...
       dvap(:ncol,k)=dvap(:ncol,k)/(1._r8-tot_water(:ncol,2))
       dliq(:ncol,k)=dliq(:ncol,k)/(1._r8-tot_water(:ncol,2))
       dice(:ncol,k)=dice(:ncol,k)/(1._r8-tot_water(:ncol,2))

   ! new surface pressure 
       state%ps(:ncol) = state%ps(:ncol) + state%pdel(:ncol,k)*(1._r8 + mdq(:ncol,k))
    end do

   ! heat and momentum exchange with condensates
    htx_cond(:ncol,:) = 0._r8 
    mxx_cond(:ncol,:) = 0._r8 
    myx_cond(:ncol,:) = 0._r8 
   ! boundary fluxes of energy and momentum due to mass sources
    eflx (:ncol) = 0._r8
    mxflx(:ncol) = 0._r8
    myflx(:ncol) = 0._r8
    if(present(eflx_ocn)) eflx_ocn(:ncol) = 0._r8

    do k = 1, pver

    ! define specific enthalpy and specific momentum of condensates (epsilon, (mox,moy))
       condmox_ref(:ncol,k) = state%u(:ncol,k)
       condmoy_ref(:ncol,k) = state%v(:ncol,k)
       condeps_ref(:ncol,k) = te(:ncol,k)+emce(:ncol,k)
       condmox    (:ncol,k) = condmox_ref(:ncol,k)
       condmoy    (:ncol,k) = condmoy_ref(:ncol,k)
       condeps    (:ncol,k) = condeps_ref(:ncol,k)

       if (present(omf_adjust)) then
        if (omf_adjust) then ! use notional surface values instead
         ! spec. mom. is bottom-level wind
         condmox(:ncol,k) = state%u(:ncol,pver)
         condmoy(:ncol,k) = state%v(:ncol,pver)
        endif  ! if omf_adjust=F, keep local winds
       endif   ! if omf_adjust not present, treated as F

       mdr(:ncol,k)=mdq(:ncol,k)
       condcp (:ncol,k) = cpcond
       if (present(ohf_adjust)) then
        if (ohf_adjust) then ! use notional surface values instead
         condcp(:ncol,k)=dvap(:ncol,k)*cpwv +dliq(:ncol,k)*cpliq+dice(:ncol,k)*cpice
         if ( present(ocnfrac) .and. present(sst) .and. present(ts) ) then
          if(adjust_to_mom) then 
           condeps(:ncol,k) =         ocnfrac(:ncol) *dvap(:ncol,k)*cpwv*sst(:ncol) &
                             + (1._r8-ocnfrac(:ncol))*dvap(:ncol,k)*cpwv*ts (:ncol) &
                             +dliq(:ncol,k)*cpliq*max(tbot(:ncol),condtr)  &
                             +dice(:ncol,k)*cpice*min(tbot(:ncol),condtr) & 
                             + state%phis(:ncol)*mdq(:ncol,k)
          else
           condeps(:ncol,k) =(ocnfrac(:ncol)*sst(:ncol)+(1._r8-ocnfrac(:ncol))*ts(:ncol))*condcp(:ncol,k)
           condeps(:ncol,k) = condeps(:ncol,k) + state%phis(:ncol)*mdq(:ncol,k)
          endif
         else if ( present(ts) ) then
          if(adjust_to_mom) then 
           condeps(:ncol,k) = dvap(:ncol,k)*cpwv *ts (:ncol) &
                             +dliq(:ncol,k)*cpliq*max(tbot(:ncol),condtr) &
                             +dice(:ncol,k)*cpice*min(tbot(:ncol),condtr) &
                             + state%phis(:ncol)*mdq(:ncol,k)
          else
           condeps(:ncol,k) = ts (:ncol)*condcp(:ncol,k)
           condeps(:ncol,k) = condeps(:ncol,k) + state%phis(:ncol)*mdq(:ncol,k)
          endif
         else if ( present(sst) ) then
          if(adjust_to_mom) then 
           condeps(:ncol,k) = dvap(:ncol,k)*cpwv *sst(:ncol) &
                             +dliq(:ncol,k)*cpliq*max(tbot(:ncol),condtr) &
                             +dice(:ncol,k)*cpice*min(tbot(:ncol),condtr) &
                             + state%phis(:ncol)*mdq(:ncol,k)
          else
           condeps(:ncol,k) = sst(:ncol)*condcp(:ncol,k)
           condeps(:ncol,k) = condeps(:ncol,k) + state%phis(:ncol)*mdq(:ncol,k)
          endif
!+18/6/24: use new input from parametrisations, with bottom T assumed for all species (regardless of phase)
! below, assume that the call follows either pure evaporation (e.g. just after coupling) or pure precip (zm, clubb)
         else if (present(evap)) then
          condeps(:ncol,k) =(cpwv*tbot(:ncol) + state%phis(:ncol))*mdq(:ncol,k) ! ok for pure addition of wv, no phase changes
         else if (present(prec) .and. present(snow) .and. present(ntprprd) .and. present(ntsnprd)) then 
         ! assume all mass is added/removed at local species-mean total enthalpy to close flux-free part of check_energy
         ! EXCEPT for (minus) the contribution to net precipitation which is assigned species-dependent bottom enthalpy
         ! to satisfy the surface-flux dependent part of check_energy.
         ! There remains a residual dependent on reserved liquid/ice which is dealt with with the column fixer.
          where(prec(:ncol).ne.0) ! prec already includes snow
           condeps(:ncol,k) =-(cpliq*tbot(:ncol)+state%phis(:ncol))*ntprprd(:ncol,k) &
                             + condeps_ref(:ncol,k)*(mdq(:ncol,k)+ntprprd(:ncol,k))
          elsewhere
           condeps(:ncol,k) =  condeps_ref(:ncol,k)* mdq(:ncol,k)
          endwhere
          where(snow(:ncol).ne.0) ! prec > snow
           condeps(:ncol,k) =  condeps(:ncol,k) &
                             -((cpice-cpliq)    *tbot(:ncol))*ntsnprd(:ncol,k) 
          endwhere
!-18/6/24
         else ! bottom temperature for everything
          condeps(:ncol,k) = condcp(:ncol,k)*tbot(:ncol)+state%phis(:ncol)*mdq(:ncol,k)
         endif
         mdr    (:ncol,k) = sign(abs(mdq(:ncol,k))+rtiny,mdq(:ncol,k))
         condeps(:ncol,k) = condeps    (:ncol,k)/mdr(:ncol,k)
         condcp (:ncol,k) = condcp     (:ncol,k)/mdr(:ncol,k)
         condeps_ref(:ncol,k) = condeps_ref(:ncol,k)/mdr(:ncol,k)*mdq(:ncol,k)
        endif  ! if ohf_adjust=F, keep local specific enthalpy
       endif   ! if ohf_adjust not present, treated as F

    ! add KE associated with momentum budget
     ! condeps    (:ncol,k) = condeps(:ncol,k) + mdq(:ncol,k)/mdr(:ncol,k)*.5 &
     !                       *(condmox(:ncol,k)**2+condmoy(:ncol,k))
    ! adjust for ocean T_0
       condeps_ocn(:ncol,k) = condeps(:ncol,k) - condcp(:ncol,k)*condtr
    ...
    ! boundary-flux diagnostic associated with water exchanged (column water-vapour gained/lost)
       mxflx(:ncol) =mxflx(:ncol) + state%pdel(:ncol,k)/gravit*condmox(:ncol,k)*mdq(:ncol,k)/dt
       myflx(:ncol) =myflx(:ncol) + state%pdel(:ncol,k)/gravit*condmoy(:ncol,k)*mdq(:ncol,k)/dt
       eflx (:ncol) = eflx(:ncol) + state%pdel(:ncol,k)/gravit*condeps(:ncol,k)*mdr(:ncol,k)/dt
       if(present(eflx_ocn)) &
       eflx_ocn(:ncol) = eflx_ocn(:ncol) + state%pdel(:ncol,k)/gravit &
                     * condeps_ocn(:ncol,k)*mdr(:ncol,k)/dt

    ! momentum and sensible heat exchange between atm. column and water reservoir
    ! heat and momentum are exchanged in the column between the local level and the surface,
    ! i.e. over indices j with (k.le.j.le.pver)
       if (present(omf_adjust)) then
        if (omf_adjust) then
         if(k.eq.1) then 
          mxx_cond(:ncol,k) = state%pdel(:ncol,k)*mdq(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              *(condmox(:ncol,k)-condmox_ref(:ncol,k))
          myx_cond(:ncol,k) = state%pdel(:ncol,k)*mdq(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              *(condmoy(:ncol,k)-condmoy_ref(:ncol,k))
         else
          mxx_cond(:ncol,k) = mxx_cond(:ncol,k-1) & ! 
                             +state%pdel(:ncol,k)*mdq(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              *(condmox(:ncol,k)-condmox_ref(:ncol,k))
          myx_cond(:ncol,k) = myx_cond(:ncol,k-1) & ! 
                             +state%pdel(:ncol,k)*mdq(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              *(condmoy(:ncol,k)-condmoy_ref(:ncol,k))
         endif
        endif ! nothing to be done if OMF_ADJUST either .false. ...
       endif  ! ... or not present

       if (present(ohf_adjust)) then
        if (ohf_adjust) then
         if(k.eq.1) then 
          htx_cond(:ncol,k) = state%pdel(:ncol,k)*mdr(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              * (condeps(:ncol,k)-condeps_ref(:ncol,k))
         else
          htx_cond(:ncol,k) = htx_cond(:ncol,k-1) & ! 
                             +state%pdel(:ncol,k)*mdr(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                              * (condeps(:ncol,k)-condeps_ref(:ncol,k))
         endif
     ...
     ! new Dp (=:Dp") for either lagrangian or hybrid-coordinate adjustment
      ! lagrangian adjustment  (Dp".eq.Dp')
       dprat(:ncol)=1._r8
       pdel_new(:ncol,k) = state%pdel(:ncol,k)*(1._r8 + mdq(:ncol,k))
       if (hybrid_coord) then 
      ! hybrid-level adjustment (Dp".ne.Dp')
        dprat(:ncol)=pdel_new(:ncol,k)
        pdel_new(:ncol,k) = (hyai(k+1)-hyai(k))*ps0 &
                           +(hybi(k+1)-hybi(k))*state%ps(:ncol)
        dprat(:ncol)=dprat(:ncol)/pdel_new(:ncol,k)
       endif

    ! add KE associated with momentum budget
     ! htx_cond(:ncol,k) = htx_cond(:ncol,k) &
     !                   +.5_r8*(dprat(:ncol)**2-1)*(state%u(:ncol,k)**2+state%v(:ncol,k)**2) &
     !                   +(mxx_cond(:ncol,k)*(dprat(:ncol)*state%u(:ncol,k)+.5_r8*mxx_cond(:ncol,k)) &
     !                    +myx_cond(:ncol,k)*(dprat(:ncol)*state%v(:ncol,k)+.5_r8*myx_cond(:ncol,k)))
...
!-----------------------------------------------------------------------
  ...
  subroutine physics_dme_adjust_THT(state, tend, qini, liqini, iceini, dt &
                                   , htx_cond, mxx_cond, myx_cond &
     !+tht 18/6/24
                              , dlf, dif &
     !-tht 18/6/24
                                   ,hydrostatic , ent_tnd, dme_hybrid_coord)
    ...
    real(r8), intent(in)               :: mxx_cond(pcols,pver)! exchange x-momentum with q's leaving/entering col 
    real(r8), intent(in)               :: myx_cond(pcols,pver)! exchange y-momentum with q's leaving/entering col 
!+18/6/24
       real(r8), intent(out), optional ::      dlf(pcols,pver)! reserved water (liq+ice) prod in layer
       real(r8), intent(out), optional ::      dif(pcols,pver)! reserved ice prod in layer
!-18/6/24
...
!-------------------- initialise adjustment loop ------------------------------------
   ...
   ! old surface pressure
    ...
    state%ps(:ncol) = state%pint(:ncol,1)
      ...
      call get_conserved_energy(.false. &               ! false for now, to be consistent with above
                               ...
                               ,latent=latent(:ncol,:),temce=emce(:ncol,:))
   ...
   ! Dp'/Dp
       ...
       mdq    (:ncol,k)=(tot_water(:ncol,2) - tot_water(:ncol,1))!/(1._r8-tot_water(:ncol,2)) 

     !+18/6/24: add back reserved liquid/ice to mdq?  
  !    if (present(dlf)) &
  !     mdq(:ncol,k)=mdq(:ncol,k)+dlf(:ncol,k)
      !if (present(dif)) &
      ! mdq(:ncol,k)=mdq(:ncol,k)+dif(:ncol,k) ! think this not required as dlf includes it
     !-18/6/24
    ...
    do k = 1, pver
     ...
     ! humidity adjustment: remapping flux from previous interface, /Dp"
       if (hybrid_coord .and. k.gt.1) then
        do m=1,pcnst
         qf (:ncol,m) =  .5_r8*(state%q(:ncol,k,m)+qtmp(:ncol,m))*edot(:ncol)/pdel_new(:ncol,k)
        enddo
       else
        do m=1,pcnst
         qf (:ncol,m) =  0._r8
     ...
     ! wind adjustment increments
       if (hybrid_coord .and. k.gt.1) then ! here u,vtmp = u,v(k-1) 
        uf (:ncol) =  .5_r8*(state%u(:ncol,k)+utmp(:ncol))*edot(:ncol)/pdel_new(:ncol,k)
        vf (:ncol) =  .5_r8*(state%v(:ncol,k)+vtmp(:ncol))*edot(:ncol)/pdel_new(:ncol,k)
       else
        uf (:ncol) = 0.
        vf (:ncol) = 0.
       endif
     ...
     ! adjust specific enthalpy
       if (hybrid_coord .and. k.gt.1) then    ! remapping flux from previous interface, /Dp"
         te (:ncol,k) = .5_r8*edot(:ncol)/pdel_new(:ncol,k) &
                       *(state%s(:ncol,k  )+emce(:ncol,k  )+latent(:ncol,k  ) &
                        +state%s(:ncol,k-1)+emce(:ncol,k-1)+latent(:ncol,k-1))
       else
         te (:ncol,k) = 0._r8
       endif
     ...
     ! momentum
       uf(:ncol) = uf(:ncol) +state%u(:ncol,k)/(fdq(:ncol)/(1._r8+mdq(:ncol,k))) +mxx_cond(:ncol,k)
       vf(:ncol) = vf(:ncol) +state%v(:ncol,k)/(fdq(:ncol)/(1._r8+mdq(:ncol,k))) +myx_cond(:ncol,k)
       ...
       if (hybrid_coord .and. k.lt.pver) then ! remapping flux from next interface, /Dp"
        ...
        te  (:ncol,k) = te(:ncol,k) - .5_r8*edot(:ncol)/pdel_new(:ncol,k) &
                       *(state%s(:ncol,k  )+emce(:ncol,k  )+latent(:ncol,k  ) &
                        +state%s(:ncol,k+1)+emce(:ncol,k+1)+latent(:ncol,k+1))
       endif
       ...
       if (hybrid_coord .and. k.lt.pver) then ! remapping flux from next interface, /Dp"
        do m=1,pcnst
         qf (:ncol,m) =  qf(:ncol,m) &
                           - .5_r8*(state%q(:ncol,k,m)+state%q(:ncol,k+1,m))*edot(:ncol)/pdel_new(:ncol,k)
        enddo
        uf (:ncol) = uf(:ncol) - .5_r8*(state%u(:ncol,k)+state%u(:ncol,k+1))*edot(:ncol)/pdel_new(:ncol,k)
        vf (:ncol) = vf(:ncol) - .5_r8*(state%v(:ncol,k)+state%v(:ncol,k+1))*edot(:ncol)/pdel_new(:ncol,k)
       endif
...
!------------------- end adjustment loop --------------------------------------------
    ...
    call inv_conserved_energy(.false. &               ! false for now, to be consistent with above
                             ,1 ,pver &
                             ,te(:ncol,:), cpairv(:ncol,:,lchnk) & 
                             ,state%q(:ncol,:,:) ,state%pdel(:ncol,:) &
                             ,pdel_new(:ncol,:) ,tp(:ncol,:) &
                             ,latent=latent(:ncol,:)*0._r8 &
                             ,phis=state%phis(:ncol) ,gph=state%zm(:ncol,:) &
                             ,U=state%u(:ncol,:) ,V=state%v(:ncol,:))
