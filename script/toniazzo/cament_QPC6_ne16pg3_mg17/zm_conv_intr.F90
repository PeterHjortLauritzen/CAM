...
!+tht 18/6/24
   real(r8), intent(out) :: ntprprd(pcols,pver)    ! evap outfld: net precip production in layer
   real(r8), intent(out) :: ntsnprd(pcols,pver)    ! evap outfld: net snow production in layer
!-tht
   ! Local variables
   character(len=512) :: errmsg
   integer            :: errflg
...
!  real(r8) :: ntprprd(pcols,pver)    ! evap outfld: net precip production in layer !tht c'd out 18/6/24
!  real(r8) :: ntsnprd(pcols,pver)    ! evap outfld: net snow production in layer   !tht c'd out 18/6/24
