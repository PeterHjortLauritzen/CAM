...
!-------------------------------------------------------------------------------
  ...
  logical  :: levels_are_moist=.true. !+tht
...
! optional args
      ...
      real(r8), intent(out), optional :: dcp_heat(pcols)     ! heat associated with dcp 
      ...
      logical , intent(in) , optional :: update_state, first_step
...
! local work space 
      ...
      if (present(dme_energy_adjust)) then
       if (dme_energy_adjust) then
        if (present(eflx).and.present(mflx)) then ! use prescribed surface enthalpy flux
          call physics_dme_bflx(state, tend, qini, liqini, iceini, dt &
                               , htx_cond, mdq, step &
                               , mflx=mflx, eflx=eflx &
                               , eflx_out=eflx_out , dcp_heat=dcp_heat &
                               , update_state=update_state, first_step=first_step )
          ncol  = state%ncol
          if (maxval(abs(eflx_out(:ncol)-eflx(:ncol))).gt.rtiny) then ! sanity check
            icol=maxloc(abs(eflx_out(:ncol)-eflx(:ncol)),1)
            write(*,'(a21,i4,3e14.6)')'flux error ('//trim(step)//'): ',icol,mflx(icol),eflx(icol),eflx_out(icol)
           !stop
          endif
        else
          call physics_dme_bflx(state, tend, qini, liqini, iceini, dt &
        ...
        if(.not.(present(update_state).and.present(first_step))) then
...
!-----------------------------------------------------------------------
  subroutine physics_dme_bflx(state, tend, qini, liqini, iceini, dt, htx_cond, mdq &
                             , step , eflx_out , dcp_heat &
                             , mflx, eflx &
                             , update_state, first_step)
    ...
    real(r8), intent(out)           :: dcp_heat(pcols)         ! diagnostic: heat associated with dcp 
    ...
    logical , intent(in) , optional :: update_state, first_step! Flags for for using accumulator, and how
    ...
    real(r8) ::    dcp     (pcols,pver)  ! species-increment-weighted cp
   ...
   ! store old pressure
    ...
    if (present(update_state)) then
     if (update_state) state%ps(:ncol) = state%pint(:ncol,1) 
    else
                       state%ps(:ncol) = state%pint(:ncol,1) 
    endif

   ! get "physics" energy
      call get_conserved_energy(levels_are_moist &   !should be consistent with physics_update
                               ,1 ,pver &
                               ,cpairv(:ncol,:,lchnk) & 
   ...
   ! compute cp change
     dcp(:ncol,:pver)=-cp_or_cv_dycore(:ncol,:pver,lchnk)
     call cam_thermo_water_update(state%q(:ncol,:,:), lchnk, ncol, vc_dycore, &
                 to_dry_factor=state%pdel(:ncol,:)/state%pdeldry(:ncol,:))
     dcp(:ncol,:pver)=dcp(:ncol,:pver)+cp_or_cv_dycore(:ncol,:pver,lchnk)
     dcp(:ncol,:pver)=dcp(:ncol,:pver) !*state%pdeldry(:ncol,:pver)/state%pdel(:ncol,:pver)
   ...
   ! heat associated with cp change
    ...
    ! mass increments Dp'/Dp
       ...
       dcp_heat(:ncol)=dcp_heat(:ncol)+ dcp(:ncol,k)*state%pdel(:ncol,k)/gravit*(state%t(:ncol,k)-condtr)/dt
 ...
 if (masterproc) then ! for testing 
   ! check for mass-less enthalpy; if found somewhere, complain
    if (present(eflx)) then 
      is_invalid(:ncol)=0
      where(eflx(:ncol).ne.0._r8.and.dcwat(:ncol).eq.0._r8)
        is_invalid(:ncol)=1
      endwhere
   ...
   ! require match between boundary mass flux (if provided) and total column water change
    if (present(mflx)) then
      if (any(abs(mflx(:ncol)+dcwat(:ncol)/dt).gt.rtiny)) then
        k=maxloc(abs(mflx(:ncol)*dt+dcwat(:ncol)),1)
 if (masterproc) &    ! for testing 
 ...
 if (masterproc) &    ! for testing 
    ...
    do k = 1, pver
      condeps_ref(:ncol,k) = te(:ncol,k)+emce(:ncol,k) - cpairv(:ncol,k,lchnk)*condtr ! local specific enthalpy
      condeps    (:ncol,k) = 0._r8                     ! spec. enthalpy of bottom mass flux
      if(present(eflx))then
        where(dcwat(:ncol).ne.0._r8.and.is_invalid(:ncol).eq.0) 
         condeps (:ncol,k) = eflx(:ncol)*dt/dcwat(:ncol)
        endwhere
      else
        where(dcwat(:ncol).ne.0._r8)
         condcp  (:ncol,k) = (dcvap(:ncol)*cpwv +dcliq(:ncol)*cpliq+dcice(:ncol)*cpice)/dcwat(:ncol)
         condeps (:ncol,k) = (condcp(:ncol,k)*(tbot(:ncol)-condtr)+state%phis(:ncol))
        endwhere
      endif
    ! make condeps's incremental and add dcp heat to condeps_ref
      condeps    (:ncol,k) = condeps    (:ncol,k)*mdq(:ncol,k)
      condeps_ref(:ncol,k) = condeps_ref(:ncol,k)*mdq(:ncol,k) + dcp(:ncol,k)*(state%t(:ncol,k)-condtr)
    end do

   ! heat exchange with condensates
    htx_cond(:ncol,:) = 0._r8 
   ! boundary flux of energy due to mass sources
    eflx_out(:ncol  ) = 0._r8
    do k = 1, pver !(n.b. removed mdq factors, now in condeps)
    ! boundary-flux diagnostic associated with water exchanged (column water gained/lost)
      eflx_out(:ncol) = eflx_out(:ncol) + state%pdel(:ncol,k)/gravit*condeps(:ncol,k)/dt
    ! diff. between local enthalpy and bottom enthalpy (or zero) is distributed in column below
      if(k.eq.1) then 
       htx_cond(:ncol,k) = state%pdel(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                           * (condeps(:ncol,k)-condeps_ref(:ncol,k))
      else
       htx_cond(:ncol,k) = htx_cond(:ncol,k-1) & ! 
                          +state%pdel(:ncol,k)/(state%ps(:ncol)-state%pint(:ncol,k)) & 
                           * (condeps(:ncol,k)-condeps_ref(:ncol,k))
      endif
  ...
  ! original pressure 
    ...
    if (present(update_state)) then     ! if not set, update state without accumulating
     if (present(first_step)) then      ! idem, if we don't know what step we're at
      if (first_step) then        ! we know what to do: initialise it...
       do k=1,pver
        tend%s_dme (:ncol,k) = htx_cond(:ncol,k)
        tend%qt_dme(:ncol,k) = mdq     (:ncol,k)
       enddo
      else
       if (update_state) then     ! ...use it...
        do k=1,pver
         htx_cond  (:ncol,k) = htx_cond(:ncol,k) + tend%s_dme (:ncol,k)*state%pdel(:ncol,k)/pdel_new(:ncol,k)
         mdq       (:ncol,k) = mdq     (:ncol,k) + tend%qt_dme(:ncol,k)
        enddo
        tend%s_dme (:,:) = 0._r8
        tend%qt_dme(:,:) = 0._r8
       else                       ! ...or keep accumulating
        do k=1,pver
         tend%s_dme (:ncol,k) = htx_cond(:ncol,k) + tend%s_dme (:ncol,k)
         tend%qt_dme(:ncol,k) = mdq     (:ncol,k) + tend%qt_dme(:ncol,k)
        enddo
       endif
      endif
     else
...
!laitnetopoeg rof dimp etadpu osla
...
!call QNEG3 (cf physics_update) 
    do m = 1, pcnst
      if (m /= ixnumice  .and.  m /= ixnumliq .and. &
        ...
        do k = 1,pver !=ptend%top_level, ptend%bot_level (see above in physics_ptend_init)
          state%q(:ncol,k,m) = max(1.e-12_r8,state%q(:ncol,k,m))
          state%q(:ncol,k,m) = min(1.e10_r8,state%q(:ncol,k,m))
        end do
      end if
    ...
    call inv_conserved_energy(levels_are_moist &   !should be consistent with physics_update
                             ,1 ,pver &
  ...
  ! update T (including tend, cf physics_update)
    do k = 1, pver
      tend%dtdt(:ncol,k) = tend%dtdt(:ncol,k) &
                         +(tp(:ncol,k) - state%t(:ncol,k))/dt
