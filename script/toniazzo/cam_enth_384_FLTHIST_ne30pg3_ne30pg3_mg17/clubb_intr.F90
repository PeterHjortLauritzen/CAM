    ...
    do k=1,pver
      do i=1, ncol
        clubb_s(i,k) = cpairv(i,k,lchnk) * thlm(i,k) / inv_exner_clubb(i,k) &
                     ! + latvap * rcm(i,k) & !tht: term seems wrong in dry s.e., and => rtm*(Lv+Li) in te_a below
...
!tht: the following comment is incorrect as zm does provide DIF; RLIQ is in fact the sum of DLF+DIF.
!  One could, perhaps should consider assigning DIF to Q(ixcldice) and avoid some of the arbitrariness
!  in this code.
  ...
  subroutine clubb_emissions_cam (state, cam_in, flx_cnd, flx_ice, ptend)
  ...
  real(r8), intent(out) :: flx_cnd(:), flx_ice(:) !+tht

  ! --------------- !
  ! Local Variables !
  ! --------------- !
  integer  :: m, ncol ,m_cnst !+tht
  logical  :: lq(pcnst)
