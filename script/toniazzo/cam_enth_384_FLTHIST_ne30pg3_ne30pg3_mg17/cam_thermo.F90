...
!+tht
    subroutine get_conserved_energy(moist_mixing_ratio, ktop, kbot &
        ...
        , gph  &
        , U, V, W, rairv &
        , latent,potential,kinetic,temce &
        , dycore_idx)
...
! optional args:
 ...
 !N.B. either PHIS or GPH must be present
      ...
      real(r8) :: qwv (SIZE(tracer, 1),SIZE(tracer, 2)) &
                 ...
                 ,qice(SIZE(tracer, 1),SIZE(tracer, 2)), latsub
...
! optional args:
 ...
 !N.B. either PHIS or GPH must be present
    ...
    subroutine inv_conserved_energy_1hd(tracer, moist_mixing_ratio, pdel_in, &
         cp_or_cv, U, V, T, z_mid, dycore_idx, qidx, te)
      ...
      if (present(T)) then
        do kdx = 1, SIZE(tracer, 2)
          do idx = 1, SIZE(tracer, 1)
           !T(idx, kdx) =  te(idx,kdx)/(cp_or_cv(idx, kdx) * pdel(idx, kdx))
            T(idx, kdx) =  te(idx,kdx)/ cp_or_cv(idx, kdx)
          end do
        end do
      end if
      deallocate(species_idx, species_liq_idx, species_ice_idx)
    end subroutine inv_conserved_energy_1hd
!-tht
!-------------------------------------------------------------------------------
