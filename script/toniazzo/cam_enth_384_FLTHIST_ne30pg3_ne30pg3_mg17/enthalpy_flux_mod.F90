module enthalpy_flux_mod
  ...
  use cam_thermo   ,    only: condtr !+tht
...
contains
 ...
 !subroutine enthalpy_zm_tend(pbuf,state,ptend,prect_enthalpy_flux)
  subroutine enthalpy_zm_tend(pbuf,state,frain,fsnow,prect_enthalpy_flux)
    use physics_types,      only: physics_ptend, physics_ptend_init, physics_state
    use physics_buffer,     only: pbuf_get_index, pbuf_get_field, physics_buffer_desc,physics_buffer_desc
    use ppgrid,             only: pcols,pver
    use physconst,          only: gravit
   ...
   !type(physics_ptend), intent(out)   :: ptend
    real(r8),            intent(out)   :: frain(pcols), fsnow(pcols)
   ...
   !call get_prec_vars(ncol,pbuf,state%hflx_bc(:,ifrain),state%hflx_bc(:,ifsnow))
    ...
    !
    frain(:ncol) = frain(:ncol) - state%hflx_ac(:ncol,ifrain)
    fsnow(:ncol) = fsnow(:ncol) - state%hflx_ac(:ncol,ifsnow)
    state%hflx_bc(:ncol,ifrain) = frain(:ncol)
    state%hflx_bc(:ncol,ifsnow) = fsnow(:ncol)
    ...
    !
    do i=1,ncol
       state%hflx_bc(:,ihrain) = -state%hflx_bc(:ncol,ifrain)*((state%T(i,pver)-condtr)*cpliq+state%phis(i))
       state%hflx_bc(:,ihsnow) = -state%hflx_bc(:ncol,ifsnow)*((state%T(i,pver)-condtr)*cpice+state%phis(i))
    end do
...
!    call pbuf_get_field(pbuf, ttend_dp_idx, ttend_dp)
 ...
 !subroutine enthalpy_clubb_pumas_tend(pbuf,state,ptend,prect_enthalpy_flux)
  subroutine enthalpy_clubb_pumas_tend(pbuf,state,frain,fsnow,prect_enthalpy_flux)
    use physics_types,      only: physics_ptend, physics_ptend_init, physics_state
    use physics_buffer,     only: pbuf_get_index, pbuf_get_field, physics_buffer_desc,physics_buffer_desc
    use ppgrid,             only: pcols,pver
    use physconst,          only: gravit
   ...
   !call get_prec_vars(ncol,pbuf,state%hflx_ac(:,ifrain),state%hflx_ac(:,ifsnow))
    ...
    ! subtract precipitation from tphys_ac from last time-step
    !
    frain(:ncol) = frain(:ncol) - state%hflx_bc(:ncol,ifrain)
    fsnow(:ncol) = fsnow(:ncol) - state%hflx_bc(:ncol,ifsnow)
    state%hflx_ac(:ncol,ifrain) = frain(:ncol)
    state%hflx_ac(:ncol,ifsnow) = fsnow(:ncol)
    ...
    !
    do i=1,ncol
       state%hflx_ac(:,ihrain) = -state%hflx_ac(:ncol,ifrain)*((state%T(i,pver)-condtr)*cpliq+state%phis(i)) 
       state%hflx_ac(:,ihsnow) = -state%hflx_ac(:ncol,ifsnow)*((state%T(i,pver)-condtr)*cpice+state%phis(i))
    end do
