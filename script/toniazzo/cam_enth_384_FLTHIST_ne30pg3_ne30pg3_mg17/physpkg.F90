module physpkg
  ...
  use cam_thermo,      only: compute_enthalpy_flux, condtr
  ...
  logical, parameter :: dme_updates_step_zm=.false., dme_updates_step_evap=.false.
  ...
  integer ::  difzm_idx          = 0     !+tht: detr convective cloud ice   mixing ratio. 18/6/24
...
!=======================================================================
  ...
  subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_in, cam_out )
    ...
    difzm_idx = pbuf_get_index('DIFZM', ierr)!+tht 18/6/24
...
!-tht
    ...
    use cam_thermo,         only: cam_thermo_water_update !,cam_thermo_water_update_conserve
    ...
    real(r8) dif(pcols,pver)                   ! Detraining cld ice from shallow + deep convections (tht)
    real(r8) rtdt                              ! 1./ztodt

    real(r8) tmp(pcols,pver) !xxx
    
    real(r8) :: rliq(pcols)! actually liq+ice? ! vertical integral of liquid not yet in q(ixcldliq)
    ...
    real(r8) :: flx_ice(pcols) !+tht

    real(r8) :: zero_sc(pcols*psubcols)        ! array of zeros
    real(r8) :: zero_tracers(pcols,pcnst)

    real(r8), pointer :: dlfzm(:,:),difzm(:,:) ! ZM detrained convective cloud water mixing ratio (tht: and ice)
    real(r8), pointer :: cmfmczm(:,:)          ! ZM convective mass fluxes
    real(r8), pointer :: rliqbc(:)             ! tphysbc reserve liquid AND ice
    ...
    real(r8) :: frain(pcols),fsnow(pcols)      ! tht: associated mass fluxes         
...
!+tht variables for dme_energy_adjust 
    integer :: m_cnst
    real(r8):: eflx(pcols), hflx_iref(pcols), dsema(pcols), dcp_heat(pcols)
    real(r8):: pdel_rf (pcols,pver)
...
!-tht
    ...
    if (difzm_idx > 0) then !+tht needed for dme_energy_adjust if calles thrice
    ...
    call clubb_emissions_cam(state, cam_in, flx_cnd, flx_ice, ptend)

    call physics_update(state, ptend, ztodt, tend)

    call check_energy_chng(state, tend, "clubb_emissions_tend", nstep, ztodt &
           !, zero, zero, zero, zero)       ! how this can be cleared with zero fluxes is a mystery
            , zero, flx_cnd, flx_ice, zero) ! (unless of course fluxes just happen to be zero) 
    ...
    if (dme_energy_adjust) then
      ...
      ! must use flux applied in coupler, passed in cam_in, not cam_out which is one time-step older
       evap_enthalpy_flux(:ncol)=cam_in%cflx(:ncol,1)*((cam_in%ts(:ncol)-condtr)*cpwv+state%phis(:ncol))
       state%q(:ncol,pver,ixq)=state%q(:ncol,pver,ixq)+cam_in%cflx(:ncol,1)*ztodt/state%pdel(:ncol,pver)*gravit

       if (dme_updates_step_evap) &
        tmp_t     (:ncol,:pver) = state%t(:ncol,:pver) !writeout diag
       call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt &
                             , dme_energy_adjust=.true.,step='evap' &
                             , update_state=dme_updates_step_evap, first_step=dme_updates_step_zm &
                             , mflx=-cam_in%cflx(:,1) , eflx=evap_enthalpy_flux &
                             , ent_tnd=dsema , dcp_heat=dcp_heat, pdel_rf=pdel_rf )

       if (dme_updates_step_evap) then
        tmp_t(:ncol,:pver) = (state%t(:ncol,:pver) - tmp_t(:ncol,:pver))/ztodt
        call outfld('PTTEND_DME', tmp_t, pcols, lchnk   )
       !call outfld('EFLX'      , dsema, pcols, lchnk) 
        call outfld('EFLX'      , dsema-dcp_heat, pcols, lchnk) !seems fine
       endif
       ! qini here includes vapour that will be added and redistributed in clubb
       qini     (:ncol,:pver) = state%q(:ncol,:pver,     ixq)
       ! restore vapour to expected pre-clubb value (N.B. safe because pdel_new/pdel>1 if cflx>0)
       state%q(:ncol,pver,ixq)=state%q(:ncol,pver,ixq)-cam_in%cflx(:ncol,1)*ztodt/state%pdel(:ncol,pver)*gravit
       ! now we can check energy with zero water flux
       if (dme_updates_step_evap) then
        call check_energy_chng(state, tend, "enthalpy_evap_tend", nstep, ztodt, zero, zero, zero, dsema)
       !N.B. dlf and dif must be rescaled to new delp (ps has changed) to maintain water budget
        dlf  (:ncol,:pver) = dlf  (:ncol,:pver)*pdel_rf(:ncol,:pver)
        dif  (:ncol,:pver) = dif  (:ncol,:pver)*pdel_rf(:ncol,:pver)
        dlfzm(:ncol,:pver) = dlfzm(:ncol,:pver)*pdel_rf(:ncol,:pver)
        difzm(:ncol,:pver) = difzm(:ncol,:pver)*pdel_rf(:ncol,:pver)
       endif

      ! reserved water is going to reappear in clubb, so must be added to the Xini's
       cldliqini(:ncol,:pver) = state%q(:ncol,:pver,ixcldliq) + dlf(:ncol,:pver)*ztodt
      !Q: is DIF actually going to reappear in CLUBB? I can't see where that happens...
       cldiceini(:ncol,:pver) = state%q(:ncol,:pver,ixcldice) + dif(:ncol,:pver)*ztodt 
       ...
       totliqini(:ncol,:pver) = totliqini(:ncol,:pver) + dlf(:ncol,:pver)*ztodt
       toticeini(:ncol,:pver) = toticeini(:ncol,:pver) + dif(:ncol,:pver)*ztodt
   !-tht
       ...
       do macmic_it = 1, cld_macmic_num_steps
          ...
          call t_startf('macrop_tend')
             ...
             call clubb_tend_cam(state, ptend, pbuf, cld_macmic_ztodt,&
                ...
                dlf+dif, det_s, det_ice)           !tht: reserved ice needs to be added to total water

             ! Since we "added" the reserved liquid back in this routine, we need
             ! to account for it in the energy checker
             flx_cnd (:ncol) = -1._r8*rliq(:ncol)
             flx_heat(:ncol) = hflx_iref(:ncol) + det_s(:ncol) !tht sens.heat from QNEG4 for ice ref. state
             ...
             call check_energy_chng(state, tend, physparname, nstep, ztodt, &
   ...
   if (carma_do_aerosol) then
    ...
    tmp = 0.0_r8
    tmp(:ncol,:pver) = cpliq*state%pdel(:ncol,:pver)*(state%q(:ncol,:pver,ixcldliq)-cldliqini(:ncol,:pver)) &
                      /(gravit*ztodt)
    call outfld ('DCLDLIQ',tmp, pcols, lchnk)
    tmp(:ncol,:pver) = cpice*state%pdel(:ncol,:pver)*(state%q(:ncol,:pver,ixcldice)-cldiceini(:ncol,:pver)) &
                      /(gravit*ztodt)
    ...
    if (dme_energy_adjust) then
      ...
      call enthalpy_clubb_pumas_tend(pbuf,state,frain,fsnow,prect_enthalpy_flux)
      tmp_t(:ncol,:pver) = state%t(:ncol,:pver)
      call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt &
                             , dme_energy_adjust=.true.,step='clubb' &
                             , update_state=.true., first_step=dme_updates_step_evap &
                             , mflx=frain+fsnow, eflx=prect_enthalpy_flux &  
                             , ent_tnd=dsema , dcp_heat=dcp_heat ) 
      tmp_t(:ncol,:pver) = (state%t(:ncol,:pver) - tmp_t(:ncol,:pver))/ztodt
      call outfld('PTTEND_DME', tmp_t, pcols, lchnk   )
     !call outfld('EFLX'      , dsema, pcols, lchnk) 
      call outfld('EFLX'      , dsema-dcp_heat, pcols, lchnk) 
      call check_energy_chng(state, tend, "enthalpy_clubb_pumas_tend", nstep, ztodt, zero, zero, zero &
     !                      , flx_sen=prect_enthalpy_flux)
                            , flx_sen=dsema              )
      call t_stopf('enthalpy_clubb_pumas_tend')
      call outfld ('enth_flx', & 
           evap_enthalpy_flux(:ncol)+ &
     ...
     !not sure about snapshots here
      if (trim(cam_take_snapshot_after) == "physics_dme_adjust") then
        call cam_snapshot_all_outfld_tphysac(cam_snapshot_after_num, state, tend, cam_in, cam_out, pbuf,&
             fh2o, surfric, obklen, flx_heat, cmfmc, dlf, det_s, det_ice, net_flx)
      end if
     !ereh stoshpans touba erus ton
  ! cam_thermo_water changes the cp's without changing T, hance changes the local enthalpy
  ! seen by hte dycore (cp*T+Phi of moist air). This is now GOOD: it's intended to happen
  ! at the end of the physics update cycle in order to account for the dcp_heat terms used
  ! in dme_bflx.
  ! Should tend still be rescaled? I think so, since now ALL explicit T updates in physics
  ! (including those in dme_adj) are consistent with cp_dry, so too big when multiplied by
  ! the dycore cp. N.B.: if dycore_cp=cp_dry, nothing actually happens, dcp_heat=0 and the 
  ! scaling is one. Energy conservation is still ensured, BUT in this case we will have the
  ! huge T tendencies.
    ...
    !...emit chus litnu
      ...
      call cam_thermo_water_update(state%q(:ncol,:,:), lchnk, ncol, vc_dycore, &
            to_dry_factor=state%pdel(:ncol,:)/state%pdeldry(:ncol,:))
      if (vc_dycore == vc_height.or.vc_dycore == vc_dry_pressure) then !tht: rescaling not yet done
  ...
  subroutine tphysbc (ztodt, state,  &
    ...
    use physics_types,   only: physics_update, &
                               physics_state_check, &
                               dyn_te_idx
    use physics_types,   only: physics_dme_adjust, set_dry_to_wet, set_wet_to_dry !+tht
    ...
    real(r8) dlf(pcols,pver)                   ! Detraining cld H20 from shallow + deep convections
    real(r8) dif(pcols,pver)                   !+tht: detr. cld ice from shallow + deep convections 18/6/24
    ...
    real(r8), pointer :: dlfzm(:,:) !really liquid ! ZM detrained convective cloud water mixing ratio.
    real(r8), pointer :: difzm(:,:)                !+tht: ZM detr convective cloud ice   mixing ratio. 18/6/24
    real(r8), pointer :: rliqbc(:)  !liquid+ice    ! tphysbc reserve liquid
    ...
    type(check_tracers_data):: tracerint       ! energy integrals and cumulative boundary fluxes
    real(r8) :: zero_tracers(pcols,pcnst)
...
!+tht variables for dme_energy_adjust 
    real(r8) :: eflx(pcols), dsema(pcols) ,dcp_heat(pcols)
    real(r8) :: pdel_rf (pcols,pver)
    real(r8) :: tmp_t   (pcols,pver) ! tmp space +tht
!-tht
    ...
    if (dme_energy_adjust) then
      ...
      call enthalpy_zm_tend(pbuf,state,frain,fsnow,precl_enthalpy_flux)

     !tht: reserved liquid and reserved ice must be removed also from the
     !     qini's to match physical precip and associated enthalpies
      cldliqini(:ncol,:pver) = cldliqini(:ncol,:pver) - dlf(:ncol,:pver)*ztodt
      totliqini(:ncol,:pver) = totliqini(:ncol,:pver) - dlf(:ncol,:pver)*ztodt
      cldiceini(:ncol,:pver) = cldiceini(:ncol,:pver) - dif(:ncol,:pver)*ztodt
      toticeini(:ncol,:pver) = toticeini(:ncol,:pver) - dif(:ncol,:pver)*ztodt

      if (dme_updates_step_zm) &   
       tmp_t(:ncol,:pver) = state%t(:ncol,:pver) 
      call physics_dme_adjust(state, tend, qini, totliqini, toticeini, ztodt &
                            , dme_energy_adjust=.true.,step='zm' &
                            , update_state=dme_updates_step_zm, first_step=.true. &
                            , mflx=frain+fsnow &
                            , eflx=precl_enthalpy_flux &  
                            , ent_tnd=dsema , dcp_heat=dcp_heat &
                            , pdel_rf=pdel_rf )

      if (dme_updates_step_zm) then
       tmp_t(:ncol,:pver) = (state%t(:ncol,:pver) - tmp_t(:ncol,:pver))/ztodt
       call outfld('PTTEND_DME', tmp_t, pcols, lchnk   )
      !call outfld('EFLX'      , dsema, pcols, lchnk) 
       call outfld('EFLX'      , dsema-dcp_heat, pcols, lchnk) ! seems fine
      !call outfld('EFLX'      , precl_enthalpy_flux, pcols, lchnk) 
       call check_energy_chng(state, tend, "enthalpy_zm_tend", nstep, ztodt, zero, zero, zero, dsema)
      !call check_energy_chng(state, tend, "enthalpy_zm_tend", nstep, ztodt, zero, zero, zero,precl_enthalpy_flux)
       dlfzm(:ncol,:pver) = dlfzm(:ncol,:pver)*pdel_rf(:ncol,:pver)
       difzm(:ncol,:pver) = difzm(:ncol,:pver)*pdel_rf(:ncol,:pver)
      endif
