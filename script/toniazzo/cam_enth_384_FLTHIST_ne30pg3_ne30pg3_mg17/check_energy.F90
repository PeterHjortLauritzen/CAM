...
!------------------------------Arguments--------------------------------
    ...
    real(r8), intent(in   ) :: ztodt               ! 2 delta t (model time increment) (who knows what was meant by this??)
!tht: porridge of sign convention and units follows:
    real(r8), intent(in   ) :: flx_vap(:)          ! (pcols) boundary flux of vapor      INTO  atm (kg/m2/s)
    real(r8), intent(in   ) :: flx_cnd(:)          ! (pcols) boundary flux of liquid+ice OUTOF atm (m/s) (precip?)
    real(r8), intent(in   ) :: flx_ice(:)          ! (pcols) boundary flux of ice        OUTOF atm (m/s) (snow?)
    real(r8), intent(in   ) :: flx_sen(:)          ! (pcols) boundary flux of sensible heat INTO (w/m2)
       ...
       if (any(abs(te_rer(1:ncol)) > 1.E-14_r8 .or. abs(tw_rer(1:ncol)) > 1.E-10_r8)) then
          do i = 1, ncol
             ...
             if ( abs(tw_rer(i)) > 1.E-10_r8) then
                ...
                write(iulog,'(a8,i5,a9,i5,a9,i4,a7,i4)') &
                 " count: ", state%count, ", nstep: ", nstep, ", chunk: ", lchnk, ", col: ", i
                write(iulog,*) tw(i)    , tw_xpd(i)      , tend%tw_tnd(i)*ztodt &
             ...
             if (abs(te_rer(i)) > 1.E-14_r8 ) then
                ...
                write(iulog,'(a8,i5,a9,i5,a9,i4,a7,i4)') &
                 " count: ", state%count, ", nstep: ", nstep, ", chunk: ", lchnk, ", col: ", i
                write(iulog,*) te(i)    , te_xpd(i)      , tend%te_tnd(i)*ztodt  &
                             , te_dif(i), te_tnd(i)*ztodt
                write(iulog,*) " relative energy deficit: ",te_rer(i)
             endif
          end do
       end if
     end if
